...
with CoshUIRenderer(PygameBackend(screen)): # This uses the Pygame backend thus needing to pass in pygame.Surface
    with Container(direction=ROW, gap=5.0):
        Label("Press The Button to Start", width=200, height=50)
        Button("Start", id="start_btn", width=100, height=50, on_hover=hover) # Takes in a callable hover
