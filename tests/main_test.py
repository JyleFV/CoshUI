    ...
    while running:
        ...
        with CoshUIRenderer(PygameBackend(screen)):
            with Container(id="container_1", sizing=CoshSizing.FILL, layout=CoshLayout(padding=20), style=CoshStyling(background_color=(80, 75, 255)), align=CoshAlign.CENTER, justify=CoshJustify.CENTER):
                with Container(id="container_2", direction=CoshDirection.COLUMN, gap=10, align=CoshAlign.CENTER, justify=CoshJustify.CENTER):
                ...
                if is_checked.value:
                    with Container(id="container_3", mouse_filter=CoshMouseFilter.STOP, on_hover=hovered, style=CoshStyling(background_color=(255, 100, 100)), x=WIDTH/2, y=HEIGHT/2, positioning=CoshPositioning.ABSOLUTE, layout=CoshLayout(padding=20)):
